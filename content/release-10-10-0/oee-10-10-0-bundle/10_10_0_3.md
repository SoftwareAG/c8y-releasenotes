...
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">Rule validation has been significantly improved for the <i>Matching</i> dialogs in the profile editor. The new validation now evaluates the type of calculation step to derive the type of the output of the whole formula. This type is compared with the expected output and errors are reported inline in the rule editor.</td>
<td style="text-align:left">COE-441<br/>COE-508</td>
</tr>
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">It is now allowed to assign machines to multiple organisational units (such as Lines, Areas and Sites). The dialog for assigning (adding) machines to an orgisational entity now lists all unassigned machines before listing the machines already assigned to any other entity. Parent organisational entities of all assigned machines are presented in the entry selection.</td>
<td style="text-align:left">COE-547</td>
</tr>
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">Drag-and-drop in <i>Organisation</i> <i>Admin</i> section has been enabled by default, but is now limited to reordering entities within it's parent entity. The switch to enable drag-and-drop in the list header has been removed.</td>
...
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">After creating a new calculation profile and loading a configuration from an existing profile (copying a profile), it is now possible to remove and assign a different machine to the calculation profile. When assigning a new machine to a calculation profile the mappings in the matching configurations are validated. If the subscriptions of old and new machines assigned to the profiles do not match, an alert will be presented and the <i>Matching</i> tab and the profile itself will show as incomplete.</td>
<td style="text-align:left">COE-349</td>
</tr>
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">For configuration of Matching rules for calculation profiles, it is now possible to refer back to the length of the calculation interval. This is required for some availability and performance calculations (e.g. when calculating the actual production amount based on a flow rate). The <code>Interval Length</code> can now be selected from the drop-down menu in the dialog.</td>
...
<tr>
<td style="text-align:left">Microservice</td>
<td style="text-align:left">When using calculation categories, for all categories the formula of the first category was used, leading to incorrect calculation results. Calculation of categories has now been fixed.</td>
...
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">When assinging (adding) machines to an organisational unit, it's now only possible to assign complete machines with complete calculation profiles. Machines with inc</td>
...
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">When editing a Matching rule of calculation profile, the title now contains the selected type of rule.</td>
<td style="text-align:left">COE-578</td>
</tr>
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">Missing translations have been fixed in the modal dialogs to assign (add) entities to organisation units.</td>
<td style="text-align:left">COE-586</td>
</tr>
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">In the machine park overview, for each organisation unit the detailed description is now displayed directly under the name of the unit changing order with the <i>last update</i> date.</td>
...
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">When quality status events are used to calculate the quality component of OEE, events are created in Cumulocity to indicate if the quality is OK. The events are now correctly created on the calculation profile instead of the machine as this has lead to confusion for machines with multiple profiles and potentially different quality criteria.</td>
<td style="text-align:left">COE-480</td>
</tr>
<tr>
<td style="text-align:left">UI</td>
<td style="text-align:left">Formatting of dates has been fixed and unified. Dates are now correctly presented using the selected user locale.</td>
