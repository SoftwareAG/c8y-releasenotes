...
<tr>
<td>Cockpit</td>
<td>Changes made to an item in the data point library will now also apply to already existing widgets if the item has been used in their configuration. For this purpose a new way of loading data for widgets called "DynamicDetailsResolver" has been added to the Web SDK (more details can be found in the Tutorial application).
