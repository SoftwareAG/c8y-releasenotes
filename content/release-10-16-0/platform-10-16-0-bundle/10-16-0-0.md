...
<td>Inventory roles algorithms with improved performance are now enabled on all tenants by default.
In previous versions they were disabled, and customer needed to turn on feature per-tenant using tenant option or Operation team needed to turn on feature per environment.</td>
<td>MTM-45800</td>
</tr>
...
<tr>
<td>Core platform</td>
<td>The managed object delete will no longer be asynchronous. The request will be blocked until the managed object and the related data is deleted. When the status code 2xx is returned by the platform then this means that all requested data has been deleted. Prior to this change, the platform returned the status code 2xx but the operation was triggered as asynchronous in the background. When the operation was unsuccessful the managed object was left.
