...
<tr>
<td>Administration</td>
<td>Files of shared applications are now accessible to all subtenants of the owner tenant. This supports a "self-service" approach for web applications (subtenants can decide if they want to install a shared application) as opposed to the currently used concept (only a parent tenant can decide if a web application gets subscribed to a subtenant).
<br>
Technical details:
