...
|:---|:---|:---
|CIE-4200|Kubernetes|**Issue:** In situations where a Kubernetes node experiences disk pressure, the kubelet attempts to reclaim node-level resources by deleting unused pods and images. However, in some cases, this eviction strategy may not be sufficient to bring the disk pressure below the defined threshold. When this occurs, the kubelet resorts to evicting pods based on the pod eviction order and images.
