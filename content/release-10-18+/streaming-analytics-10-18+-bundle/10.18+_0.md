...
The input blocks can now receive inputs from all devices by default. You can now simplify global tasks by creating a single Analytics Builder model that works with inputs from all devices, for example, send an email for every critical `C8Y_TemperatureAlarm` that is generated. A new **All Devices** option is available for this purpose.
